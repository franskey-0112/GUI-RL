...
def nested_tensor_from_tensor_list(tensors: list[torch.Tensor], ragged_idx: int | None = None) -> torch.Tensor:
    ...
    if getattr(nested_tensor, "_ragged_idx", None) == ragged_idx:
        return nested_tensor

    # If every sample has the same shape, PyTorch can infer the first non-batch
    # dimension as ragged even when callers need a later ragged dimension
    # (notably mRoPE position_ids with per-sample shape (4, seq_len)).
...
def tensor_list_from_nested_tensor(tensor: torch.Tensor) -> list[torch.Tensor]:
    assert tensor.is_nested and tensor.layout == torch.jagged
    ragged_idx = getattr(tensor, "_ragged_idx", tensor.dim() - 1)
    cat_dim = ragged_idx - 1
    offsets = tensor.offsets()
    values = tensor.values()
    starts = offsets[:-1].detach().cpu().tolist()
    ends = offsets[1:].detach().cpu().tolist()
    return [values.narrow(cat_dim, int(start), int(end - start)) for start, end in zip(starts, ends, strict=True)]


def index_select_nested_tensor(tensor: torch.Tensor, indices: torch.Tensor) -> torch.Tensor:
    assert tensor.is_nested and tensor.layout == torch.jagged
    ragged_idx = getattr(tensor, "_ragged_idx", tensor.dim() - 1)
    cat_dim = ragged_idx - 1
    offsets = tensor.offsets()
    ...
    for idx in indices.detach().cpu().tolist():
        ...
        selected_tensors.append(values.narrow(cat_dim, start, end - start))
...
def concat_nested_tensors(tensors: list[torch.Tensor]) -> torch.Tensor:
    ...
    for tensor in tensors:
        ...
        dense_tensors.extend(tensor_list_from_nested_tensor(tensor))

    ragged_idx = getattr(tensors[0], "_ragged_idx", tensors[0].dim() - 1)
    return nested_tensor_from_tensor_list(dense_tensors, ragged_idx=ragged_idx)
...
def chunk_tensordict(td: TensorDict, chunks: int) -> list[TensorDict]:
    ...
    Note:
        PyTorch batch indexing/unbind on 3D+ jagged NestedTensors can split the
        internal ``values`` buffer along the wrong dimension. We avoid those
        code paths and slice directly by offsets.
    """
    ...
    for key in nested_keys:
        nt = td[key]
        tensors = tensor_list_from_nested_tensor(nt)
    ...
    if batch is not None:
        for key, tensor in batch.items():
            if isinstance(tensor, torch.Tensor) and not tensor.is_nested:
                ...
                data_dict[key] = index_select_nested_tensor(tensor, indices)
            else:
    ...
    for key in tensordict1.keys():
        ...
        if isinstance(val, torch.Tensor):
            if val.is_nested:
                ...
                t1, t2 = tensor_list_from_nested_tensor(val), tensor_list_from_nested_tensor(val2)
